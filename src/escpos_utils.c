...
unsigned char *create_escpos_raster_command(unsigned char *bitmap_data,
  ...
  *cmd_size = 10 + bitmap_size; // 6 bytes for header + 2 bytes each for x and y
                                // + bitmap data + 2 bytes for footer
  ...
  if (cut_mode != CUT_MODE_NONE) {
    command[idx++] = 0x1D; // GS
    command[idx++] = 'V';  // V
    if (cut_mode == CUT_MODE_FULL) {
      command[idx++] = 0; // Full cut
    } else {
      command[idx++] = 1; // Partial cut (default)
    }
  }
